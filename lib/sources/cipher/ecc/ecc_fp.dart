...
import 'package:Lab_RekanKerjaku_FrontEnd/sources/bignum/src/big_integer_v8.dart';
import "package:Lab_RekanKerjaku_FrontEnd/sources/cipher/api.dart";
import "package:Lab_RekanKerjaku_FrontEnd/sources/cipher/ecc/ecc_base.dart" hide ECFieldElementBase, ECPointBase, ECCurveBase;
import "package:Lab_RekanKerjaku_FrontEnd/sources/cipher/ecc/ecc_base.dart" as ecc;
...
class ECFieldElement extends ecc.ECFieldElementBase {
  ...
  // final BigInteger q;
  // final BigInteger x;
  final BigIntegerV8 q;
  final BigIntegerV8 x;
  ...
  BigIntegerV8 toBigInteger() => x;

  ECFieldElement operator +( b ) => new ECFieldElement( q, (x + b.toBigInteger()) % q );
  ECFieldElement operator -( b ) => new ECFieldElement( q, (x - b.toBigInteger()) % q );
  ECFieldElement operator *( b ) => new ECFieldElement( q, (x * b.toBigInteger()) % q );
  ECFieldElement operator /( b ) => new ECFieldElement( q, (x * b.toBigInteger().modInverse(q)) % q );
...
class ECPoint extends ecc.ECPointBase {
  ...
  ECPoint(ECCurve curve, ECFieldElement x, ECFieldElement y, [bool withCompression=false] ) :
      super( curve, x, y, withCompression, null )
...
ECPoint _WNafMultiplier(ECPoint p, BigIntegerV8 k, PreCompInfo preCompInfo) {
